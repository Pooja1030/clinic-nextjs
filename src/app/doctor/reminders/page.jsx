"use client";

import { useState, useEffect } from "react";
import { fetchWithAuth } from "@/utils/api";
import toast, { Toaster } from "react-hot-toast";
import { useRouter } from "next/navigation";
import { Eye, Edit, Trash2, Plus, X } from "lucide-react";

const statusColors = {
  SENT: "bg-green-100 text-green-700",
  FAILED: "bg-red-100 text-red-700",
  PENDING: "bg-yellow-100 text-yellow-700",
};

const RemindersPage = () => {
  const [reminders, setReminders] = useState([]);
  const [patients, setPatients] = useState([]);
  const [searchTerm, setSearchTerm] = useState("");
  const [showForm, setShowForm] = useState(false);
  const [patientId, setPatientId] = useState("");
  const [message, setMessage] = useState("");
  const [remindOn, setRemindOn] = useState("");
  const [loading, setLoading] = useState(true);
  const [submitting, setSubmitting] = useState(false);

  const router = useRouter();
  const API_BASE = process.env.NEXT_PUBLIC_API_BASE_URL;

  useEffect(() => {
    const loadData = async () => {
      try {
        const [reminderData, patientData] = await Promise.all([
          fetchWithAuth(`${API_BASE}/api/reminders`),
          fetchWithAuth(`${API_BASE}/api/patients`),
        ]);
        setReminders(reminderData);
        setPatients(patientData);
      } catch {
        toast.error("Failed to load reminders.");
      } finally {
        setLoading(false);
      }
    };
    loadData();
  }, []);

  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!patientId || !message || !remindOn) {
      return toast.error("All fields are required");
    }

    try {
      setSubmitting(true);
      await fetchWithAuth(`${API_BASE}/api/reminders`, "POST", {
        patientId,
        message,
        remindOn,
      });
      const updated = await fetchWithAuth(`${API_BASE}/api/reminders`);
      setReminders(updated);

      setPatientId("");
      setMessage("");
      setRemindOn("");
      setShowForm(false);

      toast.success("Reminder added successfully");
    } catch {
      toast.error("Failed to add reminder");
    } finally {
      setSubmitting(false);
    }
  };

  const handleDelete = async (id) => {
    if (!confirm("Are you sure you want to delete this reminder?")) return;
    try {
      await fetchWithAuth(`${API_BASE}/api/reminders/${id}`, "DELETE");
      setReminders((prev) => prev.filter((r) => r._id !== id));
      toast.success("Reminder deleted");
    } catch {
      toast.error("Failed to delete reminder");
    }
  };

  const filteredReminders = reminders.filter(
    (r) =>
      r.patientId?.name?.toLowerCase().includes(searchTerm.toLowerCase()) ||
      r.message?.toLowerCase().includes(searchTerm.toLowerCase())
  );

  return (
    <>
      <Toaster position="top-right" toastOptions={{ duration: 3000 }} />

      <div className="max-w-7xl mx-auto px-4 py-10 space-y-8">
        {/* Header */}
        <div className="flex justify-between items-center">
          <h1 className="text-3xl font-bold text-blue-700">Reminders</h1>
          <button
            onClick={() => setShowForm(!showForm)}
            className="flex items-center gap-2 bg-gradient-to-r from-green-500 to-green-600 text-white px-5 py-2 rounded-lg shadow hover:from-green-600 hover:to-green-700 transition-all"
          >
            {showForm ? <X size={18} /> : <Plus size={18} />}
            {showForm ? "Cancel" : "Add Reminder"}
          </button>
        </div>

        {/* Search */}
        <div className="bg-white p-5 rounded-xl shadow border border-gray-200">
          <div className="flex flex-col sm:flex-row gap-4 items-center">
            <div className="flex items-center border border-gray-300 rounded-lg px-3 py-2 w-full sm:w-2/3">
              <input
                type="text"
                placeholder="Search reminders by patient name or message"
                value={searchTerm}
                onChange={(e) => setSearchTerm(e.target.value)}
                className="flex-1 outline-none"
              />
            </div>
          </div>
        </div>

        {/* Add Reminder Form */}
        {showForm && (
          <form
            onSubmit={handleSubmit}
            className="bg-white p-6 rounded-xl shadow-md border border-gray-200 space-y-6"
          >
            <div className="grid sm:grid-cols-3 gap-6">
              <select
                value={patientId}
                onChange={(e) => setPatientId(e.target.value)}
                className="w-full border border-gray-300 rounded-lg p-2 focus:outline-none focus:ring-2 focus:ring-blue-400"
                required
              >
                <option value="">Select Patient</option>
                {patients.map((p) => (
                  <option key={p._id} value={p._id}>
                    {p.name}
                  </option>
                ))}
              </select>

              <input
                type="text"
                placeholder="Reminder message"
                value={message}
                onChange={(e) => setMessage(e.target.value)}
                className="w-full border border-gray-300 rounded-lg p-2 focus:outline-none focus:ring-2 focus:ring-blue-400"
                required
              />

              <input
                type="date"
                value={remindOn}
                onChange={(e) => setRemindOn(e.target.value)}
                className="w-full border border-gray-300 rounded-lg p-2 focus:outline-none focus:ring-2 focus:ring-blue-400"
                required
              />
            </div>

            <div className="text-right">
              <button
                type="submit"
                disabled={submitting}
                className="bg-gradient-to-r from-blue-500 to-blue-600 text-white px-6 py-2 rounded-lg shadow hover:from-blue-600 hover:to-blue-700 transition-all disabled:opacity-50"
              >
                {submitting ? "Saving..." : "Save Reminder"}
              </button>
            </div>
          </form>
        )}

        {/* Reminders Table */}
        {loading ? (
          <p className="text-gray-500 text-center">Loading reminders...</p>
        ) : filteredReminders.length === 0 ? (
          <div className="bg-gray-50 p-6 rounded-xl text-center text-gray-500 border">
            No reminders found.
          </div>
        ) : (
          <div className="bg-white rounded-xl shadow border border-gray-200 overflow-hidden">
            <table className="w-full">
              <thead className="bg-gray-50 border-b">
                <tr>
                  <th className="p-4 text-left text-sm font-medium text-gray-600">Patient</th>
                  <th className="p-4 text-left text-sm font-medium text-gray-600">Message</th>
                  <th className="p-4 text-left text-sm font-medium text-gray-600">Remind On</th>
                  <th className="p-4 text-left text-sm font-medium text-gray-600">Status</th>
                  <th className="p-4 text-center text-sm font-medium text-gray-600">Actions</th>
                </tr>
              </thead>
              <tbody>
                {filteredReminders.map((rem) => (
                  <tr key={rem._id} className="border-b hover:bg-gray-50">
                    <td className="p-4">{rem.patientId?.name || "Unknown"}</td>
                    <td className="p-4">{rem.message}</td>
                    <td className="p-4">{rem.remindOn?.slice(0, 10)}</td>
                    <td className="p-4">
                      {rem.status && (
                        <span
                          className={`inline-block text-sm px-2 py-1 rounded-full ${
                            statusColors[rem.status] || "bg-gray-100 text-gray-700"
                          }`}
                        >
                          {rem.status}
                        </span>
                      )}
                    </td>
                    <td className="p-4 flex justify-center gap-3">
                      <button
                        onClick={() => router.push(`/doctor/reminders/${rem._id}`)}
                        className="p-2 bg-gray-100 hover:bg-gray-200 rounded-lg"
                      >
                        <Eye size={16} className="text-gray-600" />
                      </button>
                      <button
                        onClick={() => router.push(`/doctor/reminders/${rem._id}/edit`)}
                        className="p-2 bg-blue-100 hover:bg-blue-200 rounded-lg"
                      >
                        <Edit size={16} className="text-blue-600" />
                      </button>
                      <button
                        onClick={() => handleDelete(rem._id)}
                        className="p-2 bg-red-100 hover:bg-red-200 rounded-lg"
                      >
                        <Trash2 size={16} className="text-red-600" />
                      </button>
                    </td>
                  </tr>
                ))}
              </tbody>
            </table>
          </div>
        )}
      </div>
    </>
  );
};

export default RemindersPage;
